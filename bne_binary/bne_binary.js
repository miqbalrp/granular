...
var diag1, diag2, rhog1, rhog2, intnum, numg2, geng, r, v, m, D;
...
function setElementsLayout() {
	...
	divCont1.style.width = 350 + 10 + "px";
...
function simulate() {
	...
	if (tdummy > (tvibmax+tdelay)) {
		Zs = getCoefficient();
		degree = getDegree();
		tout(taCons, t.toExponential(4)+"\t"+Zs+"\t"+degree+"\n");
		tdummy = 0;
		vibOn = true;
	}
	...
	if (idata == Ndata) {
		...
		if (taOutParON == true) { // && vibOn == true) {
			if (tvib == 0) {
				tout(taOutPar, "# t\tvvib1\n");
			}
			tout(taOutPar, t.toExponential(4) +"\t"  + vvib1 + "\n");//r[int].z.toExponential(4) + "\n");
			if (t >= tend) {
				tout(taOutPar, "\n");
			}
		}
